function [adj, prize, weight, dist, xx, yy, ss, outInd, SDPLikeCost] = do_not_completeST(scUnw, alpha, Dist, root, X, Y)
    scores = scUnw*alpha;
    S = sum(scores);
    ...
    while (sum(Vertices.active == 1) > 0)        
        ...
        Vertices.potential = runSpTree(Dist, Vertices, scores);
    end
...
function [out] = runSpTree(Dist, Vertices, scores)
    act = unique(Vertices.compName(logical(Vertices.active)));
    for i = act'
        nodes = find(Vertices.compName == i);
        if length(nodes) > 2
            [Tree, ~] = graphminspantree(sparse(Dist(nodes,nodes)));
            Vertices.potential(nodes) = sum(scores(nodes)) - sum(Tree(:));
        end
    end
    out = Vertices.potential;
end
